...
Cline stores persistent custom instructions in VS Code's `global_instructions.json`
(`~/.vscode/extensions/saoudrizwan.claude-dev-*/globalStorage/`). The RIPER
installer writes RIPER rules there and in the workspace `.clinerules` file.

Auto-approval gate mapping:
- Ω₁ Research / Ω₅ Review — auto-approve read-only tool calls only. All writes
  must be manually approved even if auto-approve is enabled in Cline settings.
- Ω₂ Innovate / Ω₃ Plan — auto-approve file reads and diff previews; reject
  any write/execute tool call automatically.
- Ω₄ Execute — auto-approve writes only for files NOT listed as Ψ₁ PROTECTED
  or Ψ₂ GUARDED in `memory-bank/protection.md`. Prompt the user for Ψ₁/Ψ₂ files.

Always verify protection level before allowing Cline's auto-write to proceed.
When in doubt, downgrade to manual approval for the current file.
