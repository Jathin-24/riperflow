...
Roo Code stores project rules under the `.roo/rules/` directory. This file is
placed there by the RIPER installer. Roo reads all `.md` files in `.roo/rules/`
at session start and applies them as persistent instructions. You do NOT need to
re-read them manually — they are already active. If you need to verify the active
rule set, read `.roo/rules/` to list loaded files. State is tracked in
`.riper/state.json`; read it at conversation start to restore the last active mode.
